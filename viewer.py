...
class CorniceViewer(wx.ScrolledWindow):
    ...
    def paint_image(self):
        ...
        if not self.fit_image and self.zoom_index != self.DEFAULT_ZOOM:
            size = [ c * self.zoom_levels[self.zoom_index] for c in
                     self.pil_image.size ]
        ...
        if self.fit_image:
            ...
            if self.fit_enlarge:
                ...
                try:
                    ...
                    ratio = 0
                size = [ ratio * s for s in self.pil_image.size ]
...
##                                 self.interpolation)
    ...
    def on_char(self, event):
        ...
        nav_functions = {
            wx.WXK_NEXT: self.view_next_image,
            wx.WXK_SPACE: self.view_next_image,
            wx.WXK_PRIOR: self.view_prev_image,
