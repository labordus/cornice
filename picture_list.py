...
import os, stat, time, locale, Image
import random

import common, picture_thumbs, dircompleter
...
class PictureListCtrl(wx.ListCtrl, wxlist.ListCtrlAutoWidthMixin,
    ...
    def _set_image_list(self):
        ...
        self.down = imglist.Add(wx.Bitmap(os.path.join('icons', 'down.xpm'),
                                          wx.BITMAP_TYPE_XPM))
        for key, (name, color) in common.icons_and_colors.iteritems():
...
##                 f.close()
                ...
                self.SetStringItem(item, 1, mtime)
                size = img.filesize #info[stat.ST_SIZE]
                self.SetStringItem(item, 2, locale.format("%.0f",
                                                          float(size), 1))
                w, h = img.size
                if img.mode == '1':
                    sdepth = '1'
                    ...
                    sdepth = '16M'
                    depth = 16000000
                self.SetStringItem(item, 3, img.get_format_string())
...
##                                   self.list.get_active_item_info()[0])
    ...
    def next_random_image(self):
        def gen():
            l = range(self.total_files)
            random.shuffle(l)
            for i in l: yield i
        if self._random_iter is None:
            self._random_iter = gen()
        try:
            return self._view_image(self._random_iter.next())
