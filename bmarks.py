...
class BookMarksCtrl(wx.ListCtrl, wxlist.ListCtrlAutoWidthMixin):
    def __init__(self, parent):
        ...
        if not folder:
            folder = wx.ArtProvider_GetBitmap(wx.ART_FOLDER, size=(16, 16))
        imglist.Add(folder)
    ...
    def load_bookmarks(self):
        ...
        for line in bm:
            ...
            item = self.InsertImageItem(item+1, fileops.get_icon_index(path))
            self.SetStringItem(item, 1, name.strip())
            self.SetStringItem(item, 2, path.strip())
