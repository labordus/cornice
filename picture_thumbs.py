...
class PictureThumbs(wx.ScrolledWindow):
    ...
    def on_size(self, event):
        self._width = event.GetSize()[0] - \
                      wx.SystemSettings_GetMetric(wx.SYS_VSCROLL_X) - XGAP * 2
...
##                     finally: f.close()
    ...
    def _sort_filenames(self, dir_list):
        ...
        if index == common.SORT_NAME:
            dir_list = zip([i.name for i in dir_list], dir_list) #dir_list[:]
            dir_list.sort()
...
##                          for name in dir_list]
            if index == common.SORT_DATE:
                ...
                tmp = zip([i.mtime for i in dir_list], dir_list)
            elif index == common.SORT_SIZE:
                #tmp = zip([i.size for i in stat_list], dir_list)
                tmp = zip([i.filesize for i in dir_list], dir_list)
            else:
    ...
    def set_all_focused(self):
        self.focused_thumbs = range(len(self.thumbs))
    ...
    def extend_focused_until(self, thumb):
        ...
        if index in old_focused:
            ...
            if self.focused_thumbs:
                ...
                last = 0
            if index < last: r = range(index+1, last)
            else: r = range(last, index)
            for i in r:
